...
export async function discoverFromSeedWithoutCrawl(
  ...
  const onRequest = (request: RequestLike): void => {
    ...
    const source: ObservedTechnicalSource = {
      url,
      resourceType,
      discoveredOn: currentPageUrl,
    };
  ...
  try {
    ...
    const pageSignals = await page.evaluate(() => {
      ...
      for (const element of Array.from(document.querySelectorAll('*'))) {
        ...
        if (srcset) {
          for (const part of srcset.split(',')) {
            addAttr(
              part.trim().split(/\s+/)[0] ?? null,
              `${element.tagName.toLowerCase()}[srcset]`,
            );
          }
        }
      }
      ...
      for (const element of Array.from(document.querySelectorAll('link[href], meta[content]'))) {
        ...
        if (tag === 'link') {
          ...
          if (value) {
            metadata.push({
              value,
              label: `link[rel=${element.getAttribute('rel') ?? ''}]`,
            });
          }
        } else {
          const property =
      ...
      const performanceUrls =
        typeof performance !== 'undefined'
          ...
          : [];
      const historyRoutes = (
        (globalThis as unknown as { __mpotHistoryRoutes?: string[] }).__mpotHistoryRoutes ?? []
      ).slice();
    ...
    for (const route of pageSignals.historyRoutes) {
      addCandidate(rawCandidates, route, finalEntryUrl, 'history_route', {
        discoveredOn: finalEntryUrl,
      });
    }
